...
export default function FanworkLiked() {
  ...
  const fetchLikedFanworks = async () => {
    try {
      const res = await fetch(`/api/fanworks?filter=liked`, {
        cache: "no-store",
        credentials: "include",
      });
  ...
  const handlePostComment = async (fanworkId) => {
    ...
    try {
      ...
      if (json.success) {
        setCommentInputs((prev) => ({ ...prev, [fanworkId]: "" }));
        fetchLikedFanworks();
      }
    } catch (error) {
        ...
        <div className="hidden lg:block w-64 p-6">
          <div className="bg-white border-4 border-[#66AC6E] rounded-3xl p-6 sticky top-24 shadow-lg">
            <ul className="space-y-3">
              <li>
                <Link href="/profile-edit" className="block px-4 py-3 rounded-xl text-[#66AC6E] hover:bg-green-50 font-medium transition-all duration-200">
                  Edit Profile
                </Link>
              </li>
              <li>
                <Link href="/manage-fanwork" className="block px-4 py-3 rounded-xl text-[#66AC6E] hover:bg-green-50 font-medium transition-all duration-200">
                  Manage Fanwork
                </Link>
              </li>
              <li>
                <Link href="/fanwork-liked" className="block px-4 py-3 rounded-xl bg-[#66AC6E] text-white font-medium shadow-md">
                  Liked Fanwork
                </Link>
              </li>
            </ul>
          ...
          <div className="max-w-6xl mx-auto">
            ...
            {likedWorks.length === 0 ? (
              <div className="text-center py-20">
                ...
                <h3 className="text-xl font-bold text-gray-600 mb-2">Belum ada karya yang disukai</h3>
                <p className="text-gray-500 mb-6">Mulai jelajahi dan like karya favorit Anda!</p>
                <Link href="/fanwork">
                  ...
                  <div key={work.id} className="bg-white rounded-3xl shadow-lg overflow-hidden hover:shadow-xl transition border border-gray-100">
                    <div className="bg-[#66AC6E] px-5 py-3 flex justify-between items-center">
                      <div className="flex items-center space-x-3">
                        ...
                        <h2 className="font-semibold text-white text-sm">{work.user?.name || "User"}</h2>
                      </div>
                    ...
                    <div className="relative w-full h-[296px] bg-gray-100 overflow-hidden">
                      {work.imageUrl ? (
                        <Image src={work.imageUrl} alt={work.title} fill className="object-cover" sizes="(max-width: 768px) 100vw, 50vw" />
                      ) : (
                    ...
                    <div className="p-5">
                      <h3 className="text-lg font-bold text-[#66AC6E] mb-2">{work.title}</h3>
                      ...
                      {work.description.length > 100 && (
                        <button onClick={() => toggleExpand(work.id)} className="text-[#66AC6E] font-semibold text-sm flex items-center space-x-1 hover:underline mb-3">
                      ...
                      <div className="mb-3">
                        <p className="text-sm text-gray-500">
                          {work.comments?.length === 0 ? "Belum ada komentar" : `${work.comments?.length || 0} komentar`}
                        </p>
                      </div>
                      ...
                      {expandedCards[work.id] && (
                        <div className="mb-4 pt-4 border-t border-gray-200">
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            />
                            <button onClick={() => handlePostComment(work.id)} className="bg-[#E3B214] hover:bg-yellow-500 text-white px-4 py-2 rounded-lg font-semibold text-sm transition shadow-md">
                              Post
                            </button>
                          </div>
                        </div>
                      ...
                      <div className="flex gap-3 mt-4">
                        <button onClick={() => handleUnlike(work.id)} className="bg-[#E85D75] hover:bg-red-600 text-white py-3 px-6 rounded-xl font-semibold transition shadow-md">
                          Unlike
                        </button>
